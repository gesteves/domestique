...
- **Location updates bust memoized athlete caches.** `update_location` / `POST /webhooks/location` (shared core: `src/services/location-sync.ts`) write the athlete profile + weather-config, then call `IntervalsClient.invalidateAthleteCaches()` so the session-memoized timezone and weather-config (`src/utils/memo.ts` has no clear method) don't go stale on the long-running server. The webhook needs `WEBHOOK_SECRET` + `GOOGLE_API_KEY` and runs synchronously (returns the result), unlike the fire-and-forget Whoop webhook.
- **`WEBHOOK_SECRET` is shared.** One secret authenticates both `POST /webhooks/location` and `POST /webhooks/regenerate-descriptions` (Bearer or `?token=`, constant-time `secureCompare`; surfaced as `config.webhookSecret`). The regenerate-descriptions webhook (`src/webhooks/regenerate-descriptions.ts` → shared core `src/services/description-regen.ts`) takes an optional `date` (YYYY-MM-DD; defaults to today in the athlete's tz) and reuses the exact per-activity path the Whoop `workout.updated` webhook uses (`maybeGenerateActivityDescription`), but for every eligible activity that day. It's fire-and-forget like the Whoop webhook (pool swims and unavailable Strava imports are skipped; activities without a Whoop match still get a description, just no 🔥 strain line).
