...
# Google APIs (Weather API + Air Quality API + Pollen API + Elevation API; same key, all four must be enabled on the GCP project)
GOOGLE_API_KEY=             # Google Cloud API key (used for Weather API, Air Quality API, Pollen API, Elevation API, and other Google services)
