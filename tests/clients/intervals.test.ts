...
describe('IntervalsClient', () => {
  ...
  describe('getActivities', () => {
    ...
    it('emits carbs_per_hour as g/h when both intake and usage are positive', async () => {
      ...
      mockFetch
        .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve(fueledActivity) })
  ...
  describe('getActivity', () => {
    ...
    it('should fetch single activity by ID', async () => {
      ...
      const callUrl = mockFetch.mock.calls[0][0] as string;
      expect(callUrl).toContain('/activity/i113367711');
    ...
    it('should handle icu_ prefixed fields from single activity endpoint', async () => {
      // The /activity/{id} endpoint returns icu_ prefixed fields for power, efficiency, fitness, and energy
  ...
  describe('getTodayWellness', () => {
    ...
    it('maps nutrition macros (carbs, protein, fat_total) and omits null ones', async () => {
      const mockProfile = { athlete: { id: 'test', timezone: 'America/Denver' } };
      const mockWellness = {
        id: '2024-12-15',
        kcalConsumed: 2400,
      ...
      expect(result?.carbs).toBe('280 g');
  ...
  describe('stream_types optimization', () => {
    ...
    it('should fetch heat metrics when heat_strain_index is in stream_types', async () => {
      ...
      // Fifth call: intervals
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ icu_intervals: [], icu_groups: [] }),
      });

      // Sixth call: intervals streams (heat_strain_index)
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve([]),
      });

      // Seventh call: profile (for timezone)
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve(mockProfile),
      });
      ...
      // Should have made 7 fetch calls (activity + sport-settings + heat streams + notes + intervals + intervals streams + profile)
    ...
    it('should skip heat metrics fetch when heat_strain_index is not in stream_types', async () => {
      ...
      // Fourth call: intervals
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ icu_intervals: [], icu_groups: [] }),
      });

      // Fifth call: intervals streams
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve([]),
      });
      ...
      // Should have made 6 fetch calls (activity + sport-settings + notes + intervals + intervals streams + profile)
    ...
    it('should fetch temperature metrics when temp is in stream_types', async () => {
      ...
      // Sixth call: intervals streams
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve([]),
      });

      // Seventh call: weather (outdoor Run)
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ description: 'Sunny, 22°C' }),
      });

      // Eighth call: profile (for timezone)
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve(mockProfile),
      });
      ...
      // Should have made 8 fetch calls (activity + sport-settings + temp streams + notes + intervals + intervals streams + weather + profile)
    ...
    it('should fetch both heat and temp when both are in stream_types', async () => {
      ...
      // Sixth call: intervals
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ icu_intervals: [], icu_groups: [] }),
      });

      // Seventh call: intervals streams
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve([]),
      });

      // Eighth call: weather (outdoor Run)
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ description: 'Sunny, 28°C' }),
      });

      // Ninth call: profile (for timezone)
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve(mockProfile),
      });
      ...
      // Should have made 9 fetch calls (activity + sport-settings + heat streams + temp streams + notes + intervals + intervals streams + weather + profile)
