...
- `get_todays_forecast` - Today's weather forecast for each enabled location in the athlete's Intervals.icu weather config, sourced from the Google Weather API. Includes current conditions, the remaining daylight hours of the day, active alerts, local AQI (from the Google Air Quality API) on the current conditions and each hourly entry, and today's pollen forecast (from the Google Pollen API) on the current conditions.
